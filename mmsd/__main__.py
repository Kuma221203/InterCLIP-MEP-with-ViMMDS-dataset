...
class MMSDCLI(LightningCLI):
    ...
    def after_fit(self) -> None:
            ...
            if self.config.fit["search_memo_size"]:
                ...
                for memo_size in memo_sizes:
                    ...
                    if memo_rv_file is not None:
                        rv = result[0]
                        acc = round(rv["test_macro/MulticlassAccuracy"] * 100, 2)
                        f1 = round(rv["test_macro/MulticlassF1Score"] * 100, 2)
                        pr = round(rv["test_macro/MulticlassPrecision"] * 100, 2)
                        r = round(rv["test_macro/MulticlassRecall"] * 100, 2)
                    ...
                    acc = result[0]["test_macro/MulticlassAccuracy"]
                    if acc <= max_acc:
                        continue
                    max_acc = acc
                    max_rv = result[0]
                ...
                if result[0]["test_macro/MulticlassAccuracy"] <= max_acc:
                    continue
                max_acc = result[0]["test_macro/MulticlassAccuracy"]
                max_rv = result[0]
                max_ckpt = ckpt.best_model_path
        ...
        if rv_file is not None:
            ...
            acc = round(max_rv["test_macro/MulticlassAccuracy"] * 100, 2)
            f1 = round(max_rv["test_macro/MulticlassF1Score"] * 100, 2)
            p = round(max_rv["test_macro/MulticlassPrecision"] * 100, 2)
            r = round(max_rv["test_macro/MulticlassRecall"] * 100, 2)
            ...
            logger.info(f' "{cmd}"  {version_name}/{ckpt_name}   Acc: {acc}    F1_score: {f1}   Precision: {p}   Recall: {r} \n')
            rv_file.close()
