...
from torchmetrics.classification import(
                                            ...
                                            MulticlassAccuracy, MulticlassF1Score, MulticlassRecall, MulticlassPrecision
                                        )
...
class LitMMSDModel(pl.LightningModule):
    def __init__(
        ...
        while not is_success:
            try:
                self._init_model()
                is_success = True
                logger.info("Success  initialize the model")
            except Exception:
                logger.warning("Failed to initialize the model, retrying...")
                time.sleep(10)
                continue

        labels = ["not-scm", "txt-scm", "img-scm", "mul-scm"]
        macro_metrics = MetricCollection(
            [
                ...
                MetricCollection(
                    {
                        "recall" : ClasswiseWrapper(MulticlassRecall(num_classes=4, average=None), labels, prefix = "rcl_"),
                        "precision" : ClasswiseWrapper(MulticlassPrecision(num_classes=4, average=None), labels, prefix = "pcs_"),
                    }
                )
            ]
        )
        ...
        self.test_metric = macro_metrics.clone(prefix="test/")
    ...
    def _get_trainning_modules(
        ...
        for name, module in model.named_modules():
            ...
            if ("text_projection" in name or"vision_projection" in name or "classifier" in name) and isinstance(
                module, nn.Linear
            ):
                full_training_modules.append(name)
                continue
    ...
    def training_step(self, batch: MMSDModelInput) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            train_metric_step = self.train_metric(preds, batch["label"])

        self.log_dict(train_metric_step, batch_size=batch_size)
    ...
    def validation_step(self, batch: MMSDModelInput) -> None:
        ...
        self.val_metric.update(preds, batch["label"])
    ...
    def test_step(self, batch: MMSDModelInput) -> None:
        ...
        self.test_metric.update(memo_label, batch["label"])
    ...
    def predict_step(self, batch: MMSDModelInput) -> None:
        if self.predictor is None:
            raise ValueError("predictor is not initialized")
        batch_id = batch.pop("id")
        memo_pred, _, _ = self.predictor(batch)
        memo_label = torch.argmax(memo_pred, dim=-1)
        result_batch = {int(idx): self.id2label[int(lbl.item())] for idx, lbl in zip(batch_id, memo_label)}
        self.results.update(result_batch)

    def on_predict_epoch_end(self) -> None:
        sorted_results = dict(sorted(self.results.items(), key=lambda item: int(item[0])))
        results_format = {
            "results" : sorted_results,
            "phase" : "dev"
        }
        with open('results.json', 'w') as f:
            json.dump(results_format, f, indent=4)
        del self.predictor, self.results
        self.predictor = None
        self.results = None
